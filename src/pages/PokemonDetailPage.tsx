...
import { getEvolutionChain, getPokemonByType, getPokemonDetail, getPokemonSpecies } from '@/services/pokeapi'
import {
  STAT_LABELS,
  TYPE_TINTS,
  bestSprite,
...
export function PokemonDetailPage() {
  ...
  const pokemonTypeNames = detailQuery.data?.types.map((t) => t.type.name) ?? []
  ...
  const gender = speciesQuery.data ? genderRatio(speciesQuery.data.gender_rate) : null
  ...
  return (
    <div className="mx-auto max-w-3xl sm:px-4 sm:py-6">
      <div className="overflow-hidden bg-white sm:rounded-3xl sm:border sm:border-neutral-200 sm:shadow-sm">
        <div className={`flex flex-col items-center px-6 pb-8 pt-4 ${headerTint}`}>
        ...
        <div className="px-6 py-5">
          ...
          <h1 className="text-2xl font-bold text-neutral-800">{formatPokemonName(pokemon.name)}</h1>
          ...
          {flavorText && (
            <p className="mt-3 text-sm text-neutral-500">{flavorText.replace(/\f|\n/g, ' ')}</p>
          )}
          ...
          <div className="mt-5 grid grid-cols-2 divide-x divide-neutral-100 rounded-xl border border-neutral-100">
            <InfoCell icon="⚖️" label="PESO" value={`${weightToKilograms(pokemon.weight)} kg`} />
            <InfoCell icon="📏" label="ALTURA" value={`${heightToMeters(pokemon.height)} m`} />
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 divide-x divide-neutral-100 rounded-xl border border-neutral-100">
            ...
            <InfoCell
              icon="✨"
              label="HABILIDADE"
              value={primaryAbility ? formatPokemonName(primaryAbility.ability.name) : '—'}
            />
          </div>

          {gender && (
            <section className="mt-5">
              <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-neutral-500">Gênero</h2>
              <div className="flex h-2.5 overflow-hidden rounded-full bg-neutral-100">
                <div className="bg-sky-400" style={{ width: `${gender.male}%` }} />
                <div className="bg-pink-400" style={{ width: `${gender.female}%` }} />
              </div>
          ...
          {weaknesses.length > 0 && (
            <section className="mt-5">
              <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-neutral-500">Fraquezas</h2>
              <div className="grid grid-cols-2 gap-1.5">
                {weaknesses.map((type) => (
                  <TypeBadge key={type} type={type} className="justify-center" />
                ))}
              </div>
            </section>
          )}
          ...
          <section className="mt-5">
            <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-neutral-500">Estatísticas base</h2>
          ...
          <section className="mt-5">
            <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-neutral-500">Evoluções</h2>
            {evolutionQuery.data ? (
